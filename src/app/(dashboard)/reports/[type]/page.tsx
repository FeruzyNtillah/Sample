...
import { FileSpreadsheet, Download, CloudUpload, X, ArrowLeft } from 'lucide-react'
import { PageHeader } from '@/components/shared/PageHeader'
import { useDownloadTemplate, useImportReport } from '@/hooks/useReports'
...
export default function ReportTypePage({ params }: { params: { type: string } }) {
  ...
  const handleUpload = async () => {
    ...
    const result = await importReport.mutateAsync({ 
      type: params.type, 
      file: selectedFile 
    })
    
    // Redirect to wizard with import id
    router.push(`/reports/${params.type}/import/${result.id}`)
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <PageHeader
        ...
        title={formatReportType(params.type)}
        subtitle={`Upload ${formatReportType(params.type)} report Excel file`}
      />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-[#161b22] border border-[#21262d] rounded-xl p-6">
          ...
          <p className="text-sm text-gray-400 mb-6">
            Download {formatReportType(params.type)} template and fill with your data.
          </p>
          <button
            onClick={() => downloadTemplate.mutate(params.type)}
            disabled={downloadTemplate.isPending}
            className="flex items-center gap-2 px-4 py-2 border border-gray-600 text-gray-300 rounded-lg hover:bg-gray-800 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
          >
        ...
        {/* Upload Report Card */}
        <div className="bg-[#161b22] border border-[#21262d] rounded-xl p-6">
          <h3 className="text-lg font-semibold text-white mb-3">Upload Report</h3>
          <p className="text-sm text-gray-400 mb-6">
            Upload a filled Excel file with {formatReportType(params.type)} data.
          </p>
